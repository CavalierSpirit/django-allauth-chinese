...
请注意，您不一定需要 `django.contrib.auth.urls` 提供的 URLs。您可以使用 allauth 提供的 URLs：`account_logon`，`account_logout`，`account_set_password`等代替常规的 URLs 的 `login`，`logout`，`password_change`等。
